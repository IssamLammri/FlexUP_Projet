...
--
-- Hôte : 127.0.0.1:3306
-- Généré le :  ven. 10 mai 2019 à 14:08
...
CREATE TABLE IF NOT EXISTS `addresses` (
  ...
  `Is_Group` tinyint(1) NOT NULL COMMENT 'If "Yes", Owner is a "Group", Else, Owner is a "User"',
...
CREATE TABLE IF NOT EXISTS `basket` (
  ...
  `Is_Group` tinyint(1) NOT NULL COMMENT 'If "0", is a "Group", Else is a "User"',
  `ID_User` int(11) DEFAULT NULL,
  `ID_Group` int(11) DEFAULT NULL,
  `Is_Paid` tinyint(1) NOT NULL DEFAULT '0',
  `Name_Customer` varchar(50) DEFAULT NULL,
  `ID_Address` int(11) DEFAULT NULL COMMENT 'The address to which you want to send the request',
...
CREATE TABLE IF NOT EXISTS `file_proposal_group` (
  ...
  `Number_Form` int(11) NOT NULL COMMENT 'the number of the Form to be completed',
  `ID_Type_File` int(11) NOT NULL,
  `Name_Category` varchar(50) DEFAULT NULL,
  `Editor` varchar(50) NOT NULL COMMENT 'The person who made the changes',
  `Hourly_Price` int(11) DEFAULT NULL COMMENT 'The hourly price of service',
  `Price_Unit` int(11) DEFAULT NULL COMMENT 'the price of each unit',
  `Price_M2` int(11) DEFAULT NULL COMMENT ' Price per square meter',
  `Number_M2_Per_H` int(11) DEFAULT NULL COMMENT 'how much square meter achieved per hour',
  `Price_Min` int(11) DEFAULT NULL COMMENT ' the minimum price that accepted the group worked with',
...
CREATE TABLE IF NOT EXISTS `groups` (
  ...
  `Is_Flexible` tinyint(1) DEFAULT NULL COMMENT 'Can this Group offer Flexible payments to its Participants ?',
  `Is_Registered` tinyint(1) NOT NULL COMMENT 'Is this group legally registered ?',
...
CREATE TABLE IF NOT EXISTS `group_services_proposal` (
  ...
  `ID_Group` int(11) NOT NULL,
  `ID_Region` int(11) NOT NULL,
  `Proposal_Date` date NOT NULL COMMENT 'The date the company offered its service',
  `ID_File` int(11) NOT NULL COMMENT 'le formulaire exactement qui il a pour Enregistré  les propositions et calculer le Prix générale de service',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  `Name_Group` varchar(50) DEFAULT NULL COMMENT 'Name of the group you serve',
  `Quantity` int(11) NOT NULL,
  `Price` int(11) NOT NULL,
  `Urgency` varchar(50) NOT NULL COMMENT 'Either (Normal, urgent, flexible)',
  `Experience` varchar(50) NOT NULL COMMENT 'Either (Normal, Professional, Beginner)',
  `Is_Group` tinyint(1) NOT NULL COMMENT 'If "Yes", is a "Group", Else is a "User"',
  `ID_User` int(11) DEFAULT NULL,
  `ID_Group_Purchased` int(11) DEFAULT NULL,
  `ID_Group_V` int(11) NOT NULL,
  `ID_Basket` int(11) NOT NULL,
  `ID_File` int(11) NOT NULL COMMENT 'the form exactly which it has to Save the orders and calculate the General price of service',
  `ID_Service` int(11) NOT NULL,
  `ID_Region` int(11) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `orders_refused` (
  ...
  `Reason` text NOT NULL COMMENT ' Reason for refusal',
...
CREATE TABLE IF NOT EXISTS `participants` (
  ...
  `Investment_Date` date NOT NULL COMMENT 'date of the participant in the group',
  `Amount` int(11) DEFAULT NULL COMMENT 'The amount provided by the professional for the group',
  `Type` varchar(255) NOT NULL,
  `Date_Leaves` date DEFAULT NULL COMMENT 'The date the professional leaves the group',
...
CREATE TABLE IF NOT EXISTS `participants_groups` (
  ...
  `Amount` int(11) DEFAULT NULL COMMENT 'The amount provided by the professional for the group',
  `Type` varchar(255) NOT NULL,
  `Date_Leaves` date DEFAULT NULL COMMENT 'The date the group leaves the parent group',
...
CREATE TABLE IF NOT EXISTS `service_region` (
  ...
  `ID_File` int(11) NOT NULL COMMENT 'the exact form that he has to set the price of the Order',
  PRIMARY KEY (`ID`),
  KEY `ID_Service` (`ID_Service`),
  KEY `ID_Region` (`ID_Region`),
  KEY `ID_File` (`ID_File`)
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `Email` varchar(100) NOT NULL COMMENT 'the customer''s email',
  `Email_Confirmed` tinyint(1) DEFAULT NULL COMMENT '‘ 1 ’ si confirmid else ‘ 0 ‘',
  `Mobile` varchar(50) NOT NULL COMMENT 'customer''s phone number',
  `First_Name` varchar(50) NOT NULL COMMENT 'client first  name',
  `Last_Name` varchar(50) NOT NULL COMMENT 'client''s last name',
  `User_Name` varchar(100) DEFAULT NULL COMMENT 'First_Name&" "&Last_Name',
  `Password` varchar(255) NOT NULL,
  `Is_Pro` tinyint(1) DEFAULT '0' COMMENT 'Whether a User or Professional /   true  -> Pro / false -> User',
  `PID_Type` int(11) DEFAULT NULL COMMENT 'External Key to ''PID_Types'', table with list of Types of Personal Identification Document, in multiple languages (FR and EN in version 1)',
  `PID_Number` varchar(50) DEFAULT NULL,
  `PID_Expiration_Date` date DEFAULT NULL,
  `PID_Emission_Entity` varchar(100) DEFAULT NULL COMMENT 'Ex: "Ambassade de Belgique à Paris"',
  `S_S_N` int(11) DEFAULT NULL COMMENT ' social security number ',
  `ID_Main_Address` int(11) DEFAULT NULL,
  PRIMARY KEY (`ID_User`),
  KEY `ID_Main_Address` (`ID_Main_Address`),
